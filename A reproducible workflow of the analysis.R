...
tapply(X = sdsam$OTU_16,INDEX = sdsam$Group,FUN = sd )
tapply(X = sdsam$OTU_19,INDEX = sdsam$Group,FUN = sd )
tapply(X = sdsam$OTU_11,INDEX = sdsam$Group,FUN = sd )
tapply(X = sdsam$OTU_15,INDEX = sdsam$Group,FUN = sd )
tapply(X = sdsam$OTU_26,INDEX = sdsam$Group,FUN = sd )
...
tapply(X = sdneg$OTU_54,INDEX = sdneg$Group,FUN = sd)
tapply(X = sdneg$OTU_64,INDEX = sdneg$Group,FUN = sd )
...
tapply(X = sdneg$OTU_9,INDEX = sdneg$Group,FUN = sd )
tapply(X = sdneg$OTU_2,INDEX = sdneg$Group,FUN = sd )
tapply(X = sdneg$OTU_5,INDEX = sdneg$Group,FUN = sd )
tapply(X = sdneg$OTU_849,INDEX = sdneg$Group,FUN = sd )
tapply(X = sdneg$OTU_6,INDEX = sdneg$Group,FUN = sd )
tapply(X = sdneg$OTU_16,INDEX = sdneg$Group,FUN = sd )
tapply(X = sdneg$OTU_8,INDEX = sdneg$Group,FUN = sd )
tapply(X = sdneg$OTU_3,INDEX = sdneg$Group,FUN = sd )
tapply(X = sdneg$OTU_70,INDEX = sdneg$Group,FUN = sd )
tapply(X = sdneg$OTU_592,INDEX = sdneg$Group,FUN = sd )
...
#Figure S1e
plot_frequency(phy, taxa_names(phy)[c(810,1107,2127,1)], conc="Quan") + 
...
tapply(X=DNAzero$DNA,INDEX = DNAzero$Date1,FUN = max)
tapply(X=DNAzero$DNA,INDEX = DNAzero$Date1,FUN = min)
tapply(X=DNAzero$DNA,INDEX = DNAzero$Date1,FUN = mean)
tapply(X=DNAzero$DNA,INDEX = DNAzero$Date1,FUN = sd)

ggplot()+
  geom_point(DNAzero,mapping=aes(reorder(Date1,OR),DNA,group=Name,colour=Date1),size=5) +
  labs(color="Timepoints")+
  scale_color_manual(values = c('#003399','#FF3333','#FF9966','#CC6600','#330000'))+
  theme_classic()+
  geom_line(DNAzero,mapping=aes(reorder(Date1,OR),DNA,group=Name),colour='grey',alpha=0.2,size=1)+
  geom_crossbar(DNAzerot,mapping=aes(reorder(Date1,OR),DNA,ymin=DNA-SD,ymax=DNA+SD,colour=Date1),width = 0.6,size=1, position = position_dodge(0.5))+
  geom_line(DNAzeropoint, mapping=aes(reorder(Date1,OR), DNA, group = Group,colour=Date1),size=1) +
  scale_y_continuous(limits=c(-120000000,1200000000))+
  xlab("Timepoints") + ylab("DNA")+
  theme(axis.text.y=element_text(size=15),axis.title.y=element_text(size=15))+
  theme(axis.text.x=element_text(size=15),axis.title.x=element_text(size=15))+
  theme(legend.text = element_text(size=15),legend.title = element_text(size = 15))
...
tapply(X=DNAone$DNA,INDEX = DNAone$Date1,FUN = max)
tapply(X=DNAone$DNA,INDEX = DNAone$Date1,FUN = min)
tapply(X=DNAone$DNA,INDEX = DNAone$Date1,FUN = mean)
tapply(X=DNAone$DNA,INDEX = DNAone$Date1,FUN = sd)

ggplot()+
  geom_point(DNAone,mapping=aes(reorder(Date1,OR),DNA,group=Name,colour=Date1),size=5) +
  labs(color="Timepoints")+
  scale_color_manual(values = c('#003399','#FF3333','#FF9966','#CC6600','#330000'))+
  theme_classic()+
  geom_line(DNAone,mapping=aes(reorder(Date1,OR),DNA,group=Name),colour='grey',alpha=0.2,size=1)+
  geom_crossbar(DNAonet,mapping=aes(reorder(Date1,OR),DNA,ymin=DNA-SD,ymax=DNA+SD,colour=Date1),width = 0.6,size=1, position = position_dodge(0.5))+
  geom_line(DNAonepoint, mapping=aes(reorder(Date1,OR), DNA, group = Group,colour=Date1),size=1) +
  scale_y_continuous(limits=c(-60000000,900000000))+
  xlab("Timepoints") + ylab("DNA")+
  theme(axis.text.y=element_text(size=15),axis.title.y=element_text(size=15))+
  theme(axis.text.x=element_text(size=15),axis.title.x=element_text(size=15))+
  theme(legend.text = element_text(size=15),legend.title = element_text(size = 15))
...
write.csv(alpha_allt, 'D:/sputum/otu_alphat.csv', quote = FALSE)
...
p.adjust(c(8.815e-07,1.896e-06,0.6322,0.000695,0.03032,0.001945,3.156e-05,4.579e-05,1.041e-05,0.008559),method = 'bonferroni',n=10)
...
p.adjust(c(4.179e-07,5.036e-06,9.556e-05,0.001625,0.4488,0.00179,2.156e-05,0.4124,0.001492,0.03415),method = 'bonferroni',n=10)
...
ggplot()+
  ...
  annotate("text", x=1.5, y=690, label="q=8.815e-06", size=5)+
  ...
  annotate("text", x=3.5, y=690, label="q=4.579e-04", size=5)+
  ...
  annotate("text", x=2, y=750, label="q=1.896e-05", size=5)+
  ...
  annotate("text", x=2.5, y=810, label="q=6.950e-03", size=5)
...
ggplot()+
  ...
  annotate("text", x=1.5, y=7.15, label="q=4.179e-06", size=5)+
  geom_segment(aes(x=2, y=7, xend=2, yend=7.1),size=1)+
  ...
  annotate("text", x=2.5, y=7.15, label="q=9.556e-04", size=5)+
  ...
  annotate("text", x=2, y=7.35, label="q=5.036e-05", size=5)+
  ...
  annotate("text", x=2.5, y=7.55, label="q=1.625e-02", size=5)

#Figure S3 Richness and Shannon index in placebo group
...
tapply(X=richnessone$Richness,INDEX = richnessone$Date1,FUN = mean)
tapply(X=richnessone$Richness,INDEX = richnessone$Date1,FUN = sd)
...
p.adjust(c(0.06236,0.07878,0.8266,0.08116,0.3282,0.4528,0.7528,0.1522,0.1203,0.04906),method = 'bonferroni',n=10)
...
p.adjust(c(0.4617,0.2029,0.3695,0.5424,0.8489,0.6548,0.09346,0.06243,0.03825,0.09125),method = 'bonferroni',n=10)
...
adoniszero <- read.csv("D:/sputum/adonisD0.csv",header = T,row.names = 1)
samplezero =read.table("D:/sputum/sampleD0.txt", header=T, row.names= 1,sep="\t")
adonis5(adoniszero~Group,data=samplezero,permutations = 999, method="dw",tree=otu_tree) -> adt
...
#Figure S3c PCoA plot based on unweighted UniFrac distance in placebo group
...
#p.adjust(method = "bonferroni",n=4)
shapiro.test(beta4$unifrac)
wilcox.test(beta3$unifrac~beta3$Group,paired=F)
t.test(beta4$unifrac~beta4$Group,paired=F)
p.adjust(c(2.843e-09,5.181e-07,0.001907,0.07031),method = "bonferroni",n=4)
...
unifrac$Date1 <- relevel(unifrac$Date1,ref = 'D0_D4 1.1372e-08')
unifracpoint$Date1 <- relevel(unifracpoint$Date1,ref = 'D0_D4 1.1372e-08')
unifract$Date1 <- relevel(unifract$Date1,ref = 'D0_D4 1.1372e-08')
...
#Figure S3d PCoA plot based on weighted UniFrac distance in placebo group
...
#t-test:t.test(paired=F)
#p.adjust(method = "bonferroni",n=4)
shapiro.test(beta4$weight_unifrac)
wilcox.test(beta3$weight_unifrac~beta3$Group,paired=F)
t.test(beta4$weight_unifrac~beta4$Group,paired=F)
p.adjust(c(0.001881,0.2666,0.3682,0.7717),method = "bonferroni",n=4)
...
glmotuzero <- read_excel("D:/sputum/glmotuzero.xlsx")
...
glmfamzero$Date1 <- factor(glmfamzero$Date1)
fit1=lmer(Helicobacteraceae
~Date1+(1|Name),data = glmfamzero)
summary(fit1)
glmfamzero$Date1 <- relevel(glmfamzero$Date1,ref = 'D30')
#Figure 4 Heat map
...
#Figure 5a Sankey plots to describe the number of ZOTUs whose detection rate shift or return to the baseline level across the five timepoints. 
library(riverplot)
...
edges = data.frame(ID =c('F','G','H','I','J','K','L','M','N','O','P','Q','A1','A2','A3','A4'),N1 = c('A','A','Cno','A','Dno','A','A','B','B','C','C','Cno','Cno','D','D','Dno'),  
                   ...
                   Value = c(16,25,13,33,6,40,216,21,195,9,222,4,4,8,264,3),  
                   stringsAsFactors = F)  
...
#Figure 5b Sankey plots to describe the number of ZOTUs whose relative abundance shift or return to the baseline level across the five timepoints. 

edges1 = data.frame(ID =c('F','G','H','I','J','K','L','M','N','O','P','Q','A1','A2','A3','A4'),N1 = c('A','A','Cno','A','Dno','A','A','B','B','C','C','Cno','Cno','D','D','Dno'),  
                    N2 = c('Eno','E','D','D','E','C','B','Cno','C','Dno','D','Eno','E','Eno','E','Eno'),  
                    Value = c(13,11,22,12,20,36,258,38,220,25,231,5,11,20,245,5),  
                    stringsAsFactors = F)  
...
#Figure 5c For the taxa whose relative abundance have been of no significant difference from the D0 level at D14, novel shifts in their relative abundance at timepoint D30 are shown at ZOTU levels.
...
#Figure 5d For the taxa whose relative abundance have been of no significant difference from the D0 level at D14, novel shifts in their relative abundance at timepoint D30 are shown at family and genus levels.
...
#Figure S6 The daily mean air pollution data and temperature and humidity during the sputum samples collecting.
library(gtable)
library(grid) 
...
## Multivariable PERMANOVA：The relation of the environmental factors, antibiotic event and the sputum microbiota variation at D4，D30 in azithromycin
library(vegan)
library(ggplot2)
library(permute)
library(lattice)
...
source("D:/myfunction/myfuction2.R")
otu_tree <- read.tree("D:/sputum/otus.tree")
source("D:/myfunction/adonis2N.R")
#D4
...
##Multivariable PERMANOVA：The relation of the environmental factors, antibiotic event and the sputum microbiota variation at D4，D30 in placebo
#D4
...
library(ggplot2)
tra8 <- read_excel("D:/sputum/tra8.xlsx")
